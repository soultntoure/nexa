...
def initialize_dedupe_metadata(
    *,
    ...
    """Ensure dedupe metadata keys exist and include current candidate context."""
...
def merge_candidate_card(
    ...
    *,
    ...
    pattern_card["evidence_units"] = _merge_list(
        pattern_card, new_card, "evidence_units",
        keys=("type", "summary", "result", "query"), cap=5,
    )
    pattern_card["sql_findings"] = _merge_list(
        pattern_card, new_card, "sql_findings",
        keys=("query", "result"), cap=3,
        normalizer=normalize_sql_findings,
    )
    pattern_card["web_references"] = _merge_list(
        pattern_card, new_card, "web_references",
        keys=("url", "title", "snippet"), cap=2,
        normalizer=normalize_web_references,
    )
...
def _merge_list(
    ...
    *,
    ...
    """Generic merge: extract from both cards, dedupe by key fields, cap."""
    if normalizer:
